...
# plt.stem(n,np.real(yn_Diff),markerfmt = 'bo', use_line_collection=True)
plt.stem(n,np.real(yn_Idft),markerfmt = 'go', use_line_collection=True)
#plt.stem(n,np.real(yn_Ifft),markerfmt = 'ro')
plt.grid()
plt.xlabel("$n$")
plt.ylabel("$y(n)$")
plt.legend(["Difference Equation"])
plt.savefig("./figs/6_4_IDFT.pdf")
plt.show()